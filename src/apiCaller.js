 ...
 */
async function postFormData(instance,typeId = "1", plate, captcha) {
 ...
 */
async function getViolationResults(instance,typeId, plate) {
  return instance.get(`${CONFIG.RESULTS_URL}?&LoaiXe=${typeId}&BienKiemSoat=${plate}`);
}
 ...
 */callAPI
export async function callAPI(typeId, plate, retries = CONFIG.MAX_RETRIES) {
  try {
    ...
    const response = await postFormData(instance, typeId, plate, captcha);
    ...
    if (response.data === 404) {
      if (retries > 0) {
        ...
        return callAPI(typeId, plate, retries - 1);
      } else {
        throw new Error(
    ...
    const resultsResponse = await getViolationResults(instance,typeId, plate);
